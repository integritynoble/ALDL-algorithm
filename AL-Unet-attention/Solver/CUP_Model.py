...
class Depth_Decoder(Basement_TFModel):
    ...
    def __init__(self, value_sets, init_learning_rate, sess, config, is_training=True, *args, **kwargs):
        ...
        Arguments:
            ...
            initial_net: [batch, height, width, depth] phi_T_E 
        ...
        '''
        (measurement,initial_net,groundtruth,sense_mat) = value_sets
        ...
        self.decoded_image = self.encdec_handler(measurement,initial_net,groundtruth,sense_mat, 0.8)
        self.metric_opt(self.decoded_image, groundtruth)
        
            
    def encdec_handler(self, measurement, initial_net, groundtruth, sense_mat, keep_probability, 
        ...
        with tf.variable_scope(scope, 'generator', [inputs], reuse=reuse):
            with slim.arg_scope([slim.batch_norm, slim.dropout],is_training=is_training):
                with slim.arg_scope([slim.conv2d, slim.max_pool2d, slim.avg_pool2d],stride=1, padding='SAME'):
                    ...
                    self.Multiplier1.append(self.Multiplier(initial_net))
    ...
    def LinearProj_mid(self, measurement, phi_T_E, mask, linearer,shrinkage, multiplier1,multiplier2, stage):
        '''
        Input Argument:
        ...
        with tf.variable_scope('LinearProj_%d'%(stage),reuse=None):
            with slim.arg_scope([slim.variable],dtype=tf.float32,initializer=slim.initializers.xavier_initializer(),
                ...
                phi_I=tf.reduce_sum(tf.multiply(mask,linearer),axis=-1,keepdims=True)
                multipliernew2=multiplier2-lamda*(measurement-phi_I)
                auxli_v2=lamda*(tf.multiply(mask,(phi_I+multipliernew2/lamda))-phi_T_E)
